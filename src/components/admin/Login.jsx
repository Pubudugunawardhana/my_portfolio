...
import { loginAdmin, loginWithGoogleAdmin } from '../../services/authService';
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (err.message === "UNAUTHORIZED_EMAIL") {
        setError('Access Denied. Only the authorized admin email can log in.');
      } else {
        setError('Google Sign-In failed or cancelled. Please try again.');
      }
    } finally {
      setIsLoggingIn(false);
    }
      ...
      <div className="max-w-md w-full bg-white dark:bg-slate-900 rounded-3xl shadow-xl p-8 border border-slate-100 dark:border-slate-800">
          ...
          <div>
            ...
            <div className="relative">
              <input 
                ...
                className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 text-slate-900 dark:text-white transition-colors pr-12"
                placeholder="••••••••"
              />
              <button
                type="button"
        ...
        <div className="mt-6">
          <button
            ...
            disabled={isLoggingIn}
            className="w-full flex items-center justify-center gap-3 py-3 px-4 bg-white dark:bg-slate-800 text-slate-700 dark:text-white font-bold border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 shadow-sm transition-all"
          >
